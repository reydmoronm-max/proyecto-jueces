...
<body>
    ...
    <div class="body-text">
        ...
        @else
            ...
            Estado <u class="highlight">Yaracuy</u>, Teléfono <u><span class="highlight">{{ $denunciante->telefono ?? '' }}</span></u>, en adelante (el,los) Requirentes,
        @endif
        y
        @if(isset($denunciados) && $denunciados->count() > 1)
            los ciudadanos:
            @foreach($denunciados as $d)
                <u><span class="highlight">{{ mb_strtoupper($d->nombres) }} {{ mb_strtoupper($d->apellidos) }}</span></u>, de nacionalidad <u class="highlight">Venezolana</u>, titular de la cédula de identidad N° V-<u><span class="highlight">{{ $d->cedula }}</span></u>, jurídicamente hábil, domiciliado(a) en <u><span class="highlight">{{ $d->direccion }}</span></u>, Teléfono <u><span class="highlight">{{ $d->telefono }}</span></u>{{ $loop->last ? '' : '; ' }}
            @endforeach
            en carácter de Requeridos,
        @else
            (el, los), ciudadanos, (as), <u><span class="highlight">{{ mb_strtoupper($denunciado->nombres ?? '') }} 
            ...
            Estado <u class="highlight">Yaracuy</u>, Teléfono, <u><span class="highlight">{{ $denunciado->telefono ?? '' }}</span></u>, en carácter de Requerido,
        @endif
        siendo atendido (as), por el (los), ciudadanos (a), <u class="highlight">{{ mb_strtoupper($nombreJuez) }}</u>
